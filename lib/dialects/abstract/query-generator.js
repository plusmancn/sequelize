...
const QueryGenerator = {
  ...
  */
  ...
  quoteTable(param, as, partition) {
    ...
    if (partition) {
      partition = Array.isArray(partition) ? partition.map( p => this.quote(p)).join(', ') : this.quote(partition);
  ...
  */
  selectQuery(tableName, options, model) {
    ...
    mainTable.quotedName = !Array.isArray(mainTable.name) ? this.quoteTable(mainTable.name, undefined, options.partition) : tableName.map(t => {
      return Array.isArray(t) ? this.quoteTable(t[0], t[1], options.partition) : this.quoteTable(t, true, options.partition);
    }).join(', ');
