...
const QueryGenerator = {
  ...
  */
  selectQuery(tableName, options, model) {
    ...
    // Add PARTITION to sub or main query
    if (options.partition) {
      options.partition = Array.isArray(options.partition) ? options.partition.map( p => this.quote(p)).join(', ') : this.quote(options.partition);
      if (subQuery) {
        subQueryItems.push(` PARTITION(${options.partition})`);
      } else {
        mainQueryItems.push(` PARTITION(${options.partition})`);
      }
    }
